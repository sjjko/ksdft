...
arma::mat smooth(paramStruct Pa,arma::mat in);
arma::mat colsmooth(paramStruct Pa,arma::mat dat);
...
{
    int nc=dat.n_rows;
    verbosity(Pa,"colsmooth: allocate matrix, which has two times as many rows as the input matrix "+std::to_string(nc) + " and "+std::to_string(dat.n_cols) + " columns!",2,__FILE__,__LINE__);
    ...
    {
        verbosity(Pa,"colsmooth: for rowi "+std::to_string(rowi),3,__FILE__,__LINE__);
        out.row(rowo)=dat.row(rowi);
        if(rowi>0 && rowi<dat.n_rows-1 && (rowo+1)<out.n_rows)
        {
        out.row(rowo+1)=0.5*(dat.row(rowi)+dat.row(rowi+1)); //!< every second row is an interpolation of the neighbouring rows
        }
    }
    verbosity(Pa,"colsmooth: return smoothed matrix ",2,__FILE__,__LINE__);
