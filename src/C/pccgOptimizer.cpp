...
double pccgOptimizer::optimize(const int Niterations,std::shared_ptr<arma::cx_mat> Wi)
{
    ...
    verbosity("first do the setup",2,__FILE__,__LINE__);
    this->setup(Wi);
    verbosity("calculate the inital energy E0",2,__FILE__,__LINE__);
    double E0=getE(this->_Op,this->_Param,*Wi,this->_Vreal);
    verbosity("compute the initial energy gradient",2,__FILE__,__LINE__);
    arma::cx_mat g_nm1=getgrad(_Op,_Param,*Wi,_Vreal);
    verbosity("do the preconditioning",2,__FILE__,__LINE__);
    arma::cx_mat d_nm1=-this->K(g_nm1);
    verbosity("now start looping",2,__FILE__,__LINE__);

    int n=0;
    while(n<Niterations)
        {
            arma::cx_mat g_n=getgrad(this->_Op,this->_Param,*Wi,this->_Vreal);
            if(n>1)
            {
            ...
            arma::cx_mat gtrial=getgrad(this->_Op,this->_Param,Wtrial,this->_Vreal);
            double alpha=this->_alpha*Prod(g_n,d_n)/Prod(g_n-gtrial,d_n);
            *Wi=*Wi+alpha*d_n;
            Energy_at_this_level=getE(this->_Op,this->_Param,*Wi,this->_Vreal);
            g_nm1=g_n;
            d_nm1=d_n;
            cout << "====================================" << endl;
...
pccgOptimizer::pccgOptimizer(const operatorStruct Operator,const paramStruct Parameter,const arma::vec Vreal,const arma::mat G2Input,latexComment *ltX)
:optimizeBase(Operator,Parameter,Vreal)
{
    ...
    myLatexClass=ltX;
    this->_Vdual=getVdual(Operator,Vreal);
    this->_Vreal=Vreal;
